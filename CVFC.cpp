...
void man(char *name)
{
	...
	{
		printf("Description : Used to display inforamation of file\n");
...
void DisplayHelp()
{
	...
	printf("\tread : To Read the contents from fike\n");
	printf("\twrite : To write conents into the file\n");
	printf("\texit : To Terminate file system\n");
	printf("\tstat : To Display inforamation of file using name\n");
...
void CreateDILB()
{
	...
	printf("DILB created successsfully\n");
}
...
int fstat_file(int fd)
{
	...
	printf("---------Statical Inforamation about file---------\n\n");
	printf("File name : %s\n",temp -> FileName);
...
int main()
{
	...
	while(1)
	{
		...
		if(count == 1)
		{
			...
			{
				CloseAllFile();
				printf("All file closed successsfully\n");
				continue;
			}
		...
		else if (count == 2)
		{
			...
			else if(strcmp(command[0],"write") == 0)
			{
				...
				if(ret == -2)
					printf("ERROR : There is no such sufficientmemory to writ\n");
				if(ret == -3)
					printf("ERROR : It is ot regulerfile\n");
			}
		...
		else if(count == 3)
		{
			if(strcmp(command[0],"create") == 0)
			{
				...
				if(ret >= 0)
					printf("File is Sucessfully created with file descroptor : %d\n",ret);
				if(ret == -1)
					printf("ERROR : Incorrect parameters\n");
				if(ret == -2)
					printf("ERROR : File not present\n");
				if(ret == -3)
					printf("ERROR Permission Deied\n");
				if(ret == -4)
		...
		else if(count == 4)
		{
			if(strcmp(command[0],"lseek") == 0)
			{
				...
				if(ret == -1)
				{
					printf("ERORR : Unable to perform lseek\n");
				}
			}
			else
			{
